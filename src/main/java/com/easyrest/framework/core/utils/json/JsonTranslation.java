package com.easyrest.framework.core.utils.json;
...
public final class JsonTranslation {
    ...
    private static Gson gson = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(JsonContent.class, new JsonContentAdapter()).setLongSerializationPolicy(LongSerializationPolicy.STRING).create();
    private static Gson gsonWithNulls = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(JsonContent.class, new JsonContentAdapter()).serializeNulls().setLongSerializationPolicy(LongSerializationPolicy.STRING).create();
