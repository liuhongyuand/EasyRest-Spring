package com.simpacct.framework.core.model.image.util;

import com.github.cage.Cage;
import com.github.cage.GCage;
import com.simpacct.framework.core.services.session.SessionParameters;
import com.simpacct.framework.core.services.session.SessionSupport;
...
public class ImageSupport {
    ...
    public static byte[] createVerifyCode(HttpServletRequest request){
        Cage cage = new GCage();
        String token = StringUtils.getRandomCaptcha();
        try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
            cage.draw(token, byteArrayOutputStream);
            if (SessionSupport.put(request, SessionParameters.CAPTCHA.getParameter(), token)) {
