package com.simpacct.framework.core.model.file;
    ...
    private void parse(HttpServletRequest request) throws FileUploadException {
        ...
        if (multipart) {
            ...
            for (Object item1 : items) {
                ...
                if (item.isFormField()) {
                    try {
                        parameters.put(item.getFieldName(), item.getString(GlobalParameters.characterEncoding));
                    } catch (UnsupportedEncodingException ignored) {}
                } else {
                    try {
                        FileUploaded uploaded = new FileUploaded();
                        uploaded.setRealName(item.getName());
                        File file = new File(GlobalParameters.uploadFileDir + uploaded.getNewName());
                        FileUtils.writeByteArrayToFile(file, item.get());
                        uploaded.setFile(file);
                        LogUtils.info(String.format(UPLOAD_MESSAGE, uploaded.getRealName(), uploaded.getFile().getAbsolutePath(), uploaded.getNewName()));
